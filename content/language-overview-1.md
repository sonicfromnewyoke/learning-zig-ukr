...
Якщо ви збережете наведене вище як _learning.zig_ і запустите `zig run learning.zig`, ви побачите: `Потужність Goku становить 9001`.
...
___
Перегляньте розділ [встановлюємо zig](/#install"), щоб швидко розпочати роботу.
___
...
Коли ми створюємо структуру, __кожне__ поле має бути встановлено. Наприклад, у вихідному визначенні, де `power` не мав значення за замовчуванням, наступне давало б помилку: __missing struct field: power__
...
___
Якщо ви знайомі з Go, ви могли помітити, що зрізи в Zig дещо інші: вони __не__ мають ємність, лише вказівник і довжину.
___
...
___
Зауважте, ця помилка сталася лише тому, що ми використовували `var`. Якби ми використовували `const`, ми б не мали помилки, оскільки вся суть помилки полягає в тому, що `comptime_int` __має__ бути const
___
...
Виникає запитання: що __ми__ йому передаємо? Раніше ми вже бачили нотацію `.{...}`, коли компілятор міг визначити тип нашої структури. Це схоже: створюється анонімний структурний літерал. Розгляньте цей код:
