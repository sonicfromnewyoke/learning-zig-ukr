...
Початкова перевірка `if` — це не просто хороша оптимізація продуктивності, це необхідний захист. Якщо ми виймемо його та передамо аргументи різної довжини, ми отримаємо паніку під час виконання: _for loop over objects with non-equal lengths_.
...
У нашому `switch` використано три крапки, `3...6`, тоді як у цьому використовуються дві крапки, `0..10`. Це тому, що регістр `switch` включає обидва числа, тоді як `for` не включає верхню межу.
...
Це короткий огляд типів із значенням nullable.

---

Кінець діапазону визначається довжиною `haystack`, хоча ми могли б покарати себе і написати: `0..hastack.len`. Цикли `for` не підтримують більш загальний `init; compare; step` ідіома. Для цього ми покладаємося на `while`.
...
`while` може мати пункт `else`, який виконується, коли умова хибна. Він також приймає оператор для виконання після кожної ітерації. Може бути кілька операторів, розділених символом `;`. Ця функція зазвичай використовувалася до «для» підтримки кількох послідовностей. Вищесказане можна записати так:
...
Вичерпна природа `switch` робить його гарним поєднанням з переліком (`enum`), оскільки це гарантує, що ви обробили всі можливі випадки. Проте будьте обережні, використовуючи `else` гілку в `switch`, оскільки воно відповідатиме будь-яким нещодавно доданим значенням переліку, що може бути чи не відповідати бажаній поведінці.
...
Нарешті, тип переліку тегованого об’єднання можна зробити висновком. Замість того, щоб визначати `TimestampType`, ми могли б зробити:
...
Але ми отримаємо паніку під час виконання, якщо використаємо `.?` для null. Оператор `if` може безпечно розгорнути необов’язковий:
...
`orelse` можна використовувати для розгортання необов'язкового значення або виконання коду. Це зазвичай використовують для визначення значення за замовчуванням або повернення з функції:
...
Поки що кожна окрема змінна, яку ми бачили, була ініціалізована розумним значенням. Але іноді ми не знаємо значення змінної, коли її оголошено. Варіанти є одним із варіантів, але не завжди мають сенс. У таких випадках ми можемо встановити змінні на `undefined`, щоб залишити їх неініціалізованими.
...
Якщо ви спробуєте запустити це, ви отримаєте повідомлення про помилку: _expected type 'void', found 'error{AccessDenied,NotFound}'_. Це має сенс: ми визначили `main` з типом повернення `void`, але ми щось повертаємо (помилка, звичайно, але це все ще не `void`). Щоб вирішити цю проблему, нам потрібно змінити тип повернення функції.
...
Це називається типом об’єднання помилок і вказує на те, що наша функція може повертати помилку `OpenError` або `void` (він же нічого). Поки що ми були досить чіткими: ми створили набір помилок для можливих помилок, які може повернути наша функція, і використали цей набір помилок у типі повернення об’єднання помилок нашої функції. Але коли справа доходить до помилок, Zig має кілька хитрих прийомів у своєму рукаві. По-перше, замість того, щоб вказувати об’єднання помилок як `error set!return type`, ми можемо дозволити Zig вивести набір помилок за допомогою: `!return type`. Таким чином, ми могли б і, ймовірно, визначили б наш `main` як:
...
Справжньою цінністю об’єднань помилок є вбудована підтримка мови у формі `catch` і `try`. Виклик функції, яка повертає об’єднання помилок, може містити пропозицію `catch`. Наприклад, бібліотека http-сервера може мати такий код:
...
Це все досить дивно, але давайте будемо чесними, найімовірніше, що ви збираєтеся зробити в `catch`, це повідомити про помилку абоненту:
...
Це особливо корисно, враховуючи, що **помилка повинна бути оброблена**. Швидше за все, ви зробите це за допомогою "спробувати" або "зловити".
...
Здебільшого ви будете використовувати `try` і `catch`, але об'єднання помилок також підтримуються `if` і `while`, подібно до необов'язкових типів. У випадку `while`, якщо умова повертає помилку, виконується пропозиція `else`.
...
Хоча Zig має більшу глибину, а деякі мовні функції мають більші можливості, те, що ми побачили в цих перших двох частинах, є значною частиною мови. Він слугуватиме основою, що дозволить нам досліджувати складніші теми, не надто відволікаючись на синтаксис.
